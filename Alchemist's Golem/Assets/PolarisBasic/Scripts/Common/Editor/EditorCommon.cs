...
namespace Pinwheel.PolarisBasic
{
    public static class EditorCommon
    {
        ...
        public static bool GroupFoldout(bool foldOut, string name)
        {
            string leftCaret = "►";
            string downCaret = "▼";
            string label = string.Format("{0} {1}", foldOut ? downCaret : leftCaret, name);
            if (GUILayout.Button(label, GuiStyleUtilities.InspectorGroupFoldout, GUILayout.Height(EditorCommon.standardHeight)))
            {
                foldOut = !foldOut;
            }
            return foldOut;
